  ...
  *errs=0; 
  ...
  while (j<23) /* flip each trial bit */ 
    { 
      ...
      s=golay_syndrome(cw); /* look for errors */ 
      if (s) /* errors exist */ 
        { 
          for (i=0; i<23; i++) /* check syndrome of each cyclic shift */ 
            { 
              if ((*errs=golay_weight(s)) <= w) /* syndrome matches error pattern */
                { 
                  cw=cw ^ s;              /* remove errors */ 
                  cw=golay_rotate_right(cw,i);  /* unrotate data */ 
                ...
                { 
                  cw=golay_rotate_left(cw,1);   /* rotate to next pattern */ 
                  s=golay_syndrome(cw);         /* calc new syndrome */ 
                } 
            } 
