...
void send_ask_command(unsigned long command) {
    ...
    do {
       ...
       // to transmit 1 we need a transition from 0 to 1 so we set high now (1)
       // to transmit 0 we need a transition from 1 to 0 so we set low now (0)
        if (bit_to_transfer == BIT_1) current_signal_level = HIGH;
            else current_signal_level = LOW;
